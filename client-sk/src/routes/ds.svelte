<script context="module">
	...
	import { updateList, deleteItem, train, trainCNN, create } from '../api/dataset';
...
<script>
	...
	const handleTrainCNN = async () => {
		isTraining = true;
		const ret = await trainCNN();
		isTraining = false;
...
<main>
	...
	<div class="trainingBlock">
		...
		{/if}
		<button on:click={handleTrain}> Train DNN </button>
		<button on:click={handleTrainCNN}> Train CNN </button>
	</div>
			...
			<div class="dsRow">
				...
				{#each dsGroup.list as dsItem}
					<div class="dsList__img">
						...
						<img class="img_cnn_small" src={`${dsImgRoot}/prepared_cnn/${dsItem.img}`} alt="" />
						<span class="dsList__img--delete" on:click={() => delItem(dsItem.id)}> Delete </span>
					</div>
				{/each}
			</div>
		{/each}
